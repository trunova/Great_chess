...
@JsonTypeName("wm")
public class WM extends Piece { // ладьи и конь

    private final int[][] CANDIDATE_MOVE_COORDINATES_KNIGHT = {{-2, -1}, {-2, 1}, {-1, 2}, {1, 2},
                                                                {2, 1}, {2, -1}, {1, -2}, {-1, -2}};
    private final int[][] CANDIDATE_MOVE_COORDINATES_ROOK = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};

    public WM(Alliance pieceAlliance) {
        super(pieceAlliance);
    }

    public WM(Alliance pieceAlliance, int x, int y) {
        super(pieceAlliance, x, y);
    }

    public WM(List<Integer[]> canGoFromThis, List<Integer[]> canAttackThis, int x, int y, Alliance pieceAlliance) {
    ...
    public List<Move> calculateLegalMoves(final Board board) {
        ...
        for (final int[] candidateMove : CANDIDATE_MOVE_COORDINATES_ROOK) {
            int x = this.x;
            int y = this.y;
